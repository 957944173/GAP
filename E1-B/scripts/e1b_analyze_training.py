#!/usr/bin/env python3
"""E1-B training-side diagnostics + comparison against the E1-0 baseline run.

Reads the diagnostics written by the E1-B reward manager
(`<E1B_RUN_DIR>/diagnostics/{rollouts,groups,calls}_pid*.jsonl`) and, for the
...
Outputs (into --out):
    training_summary.json               total/overall metrics for E1-B (+baseline)
    ...
    training_comparison.json/md         E1-B vs E1-0 per-step and overall
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--run-dir", required=True, help="E1-B run dir containing diagnostics/")
    ...
    if not rolls:
        raise SystemExit(f"no E1-B diagnostics found under {args.run_dir}/diagnostics")

    e1a = summarise(rolls, groups, "E1-B shaped reward")
    ...
    reward_dist = {
        "e1b_reward_hist": e1a["reward_hist"],
        "e1b_em_rate": e1a["em_rate"],
        "e1b_reward_nonzero_rate": e1a["reward_nonzero_rate"],
    ...
    eff_dist = {
        "e1b_efficiency_hist": e1a["efficiency_hist"],
        "e1b_efficiency_mean": e1a["efficiency_mean"],
        "e1b_efficiency_nonzero_rate": e1a["efficiency_nonzero_rate"],
        "e1b_efficiency_active_groups": e1a.get("efficiency_active_groups"),
    }
    search_behavior = {
        ...
        "delta_e1b_minus_baseline": ({k: (e1a[k] - baseline[k]) for k in
    ...
    group_cc = {"e1b_group_correct_count_hist": e1a.get("group_correct_count_hist"),
                "e1b_group_correct_count_mean": e1a.get("group_correct_count_mean"),
                "baseline_group_correct_count_hist": (baseline or {}).get("group_correct_count_hist")}
    retention = {
        "e1b_baseline_kept_groups": e1a.get("filter_retention_baseline"),
        "e1b_shaped_kept_groups": e1a.get("filter_retention_shaped"),
        "e1b_newly_added_groups": e1a.get("filter_newly_added"),
        "e1b_efficiency_active_groups": e1a.get("efficiency_active_groups"),
        "e1b_efficiency_supervision_ratio": e1a.get("efficiency_supervision_ratio"),
    ...
    safety = {
        ...
        "zero_search": {
            "e1b_zero_search_rate": e1a.get("zero_search_rate"),
            "baseline_zero_search_rate": (baseline or {}).get("zero_search_rate"),
            "e1b_zero_search_correct_rate": (
    ...
    summary = {"e1a": e1a, "baseline": baseline,
               "e1b_per_step": per_step_table(rolls, groups),
               "e1b_calls": calls}
    for name, obj in (("training_summary.json", summary),
                      ...
                      ("training_comparison.json", {"e1a": e1a, "baseline": baseline,
                                                     "per_step_e1a": summary["e1b_per_step"]})):
    ...
    lines = ["# E1-B training-side diagnostics (step 60 -> 70)", ""]
    lines.append("| metric | E1-B (shaped) | baseline E1-0 (original EM) | delta |")
    lines.append("|---|---|---|---|")
