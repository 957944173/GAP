#!/usr/bin/env bash
# E1-B integrity re-verification: the original GAP files must be byte-identical before
# and after the whole E1-B experiment (E1-B_task.md "不要直接修改原 GAP 主代码").
#
# Recomputes the manifest written by scripts/e1b_preflight.sh and diffs it.
# Outputs: E1-B/configs/original_files_manifest_post.txt
#          E1-B/configs/original_files_diff.txt  (empty == unchanged)
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
E1B_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$E1B_ROOT/.." && pwd)"
OUT="$E1B_ROOT/configs"
mkdir -p "$OUT"
