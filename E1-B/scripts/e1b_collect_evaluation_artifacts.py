#!/usr/bin/env python3
"""Collect E1-B evaluation artifacts (E1-B_task.md section 6).

Scans every evaluation directory under E1-B/evaluations/ that contains an
`eval_summary.json` (produced by e1b_analyze_eval.py) and writes:

    E1-B/evaluations/results.json           -- full nested results (per label: accuracy,
                                               per-benchmark behavior, overall metrics)
    E1-B/evaluations/benchmark_scores.csv   -- one row per (label, benchmark): EM / n
    E1-B/evaluations/behavior_metrics.csv   -- one row per (label, benchmark) with the
                                               search-behaviour metrics
    E1-B/evaluations/overall_metrics.csv    -- one row per label, macro/micro EM + behaviour

Usage: python3 e1b_collect_evaluation_artifacts.py [--eval-root DIR] [--out DIR]
"""
import argparse
import csv
import glob
...
def main():
    ...
    for e in entries:
        ...
        results[e["name"]] = {
            ...
            "dir": os.path.relpath(e["dir"], e1b_root),
