#!/usr/bin/env python3
"""Multi-way comparison of all E1-B / reference evaluations (E1-B_task.md 8/9/10).

Reads every `eval_summary.json` under E1-B/evaluations/ (E1-B step80..120 plus the
re-aggregated references gap_step60, gap_step120, gap_step180, e1a_step70, e10_step70)
and emits:

    E1-B/evaluations/comparison/comparison.json
    E1-B/evaluations/comparison/comparison.md            (full table, all runs)
    E1-B/evaluations/comparison/vs_gap_step120.md        (E1-B step120 vs GAP baseline)
    E1-B/evaluations/comparison/vs_e1a_step70.md         (E1-B step120 vs E1-A start)
    E1-B/evaluations/comparison/per_benchmark_deltas.md  (per-benchmark behaviour deltas)
...
def main():
    ...
    root = args.eval_root or os.path.join(e1b_root, "evaluations")
    ...
    e1b_steps = {int(k.split("_")[0][4:]): k for k in runs if k.startswith("step")}

    result = {"runs": runs, "roles": {"gap_step120": gap120, "gap_step60": gap60,
                                      "gap_step180": gap180, "e1a_step70": e1a70,
                                      "e10_step70": e10_70,
                                      "e1b_steps": {str(k): v for k, v in sorted(e1b_steps.items())}}}
    ...
    result["vs_gap_step120"] = delta_block(gap120, e1b_steps.get(120))
    result["vs_e1a_step70"] = delta_block(e1a70, e1b_steps.get(120))
    result["vs_gap_step60"] = delta_block(gap60, e1b_steps.get(120))
    result["vs_e10_step70"] = delta_block(e10_70, e1b_steps.get(120))
    # trajectory of the E1-B branch, anchored at its own start (E1-A step70)
    traj = {}
    for s in sorted(e1b_steps):
        traj[s] = delta_block(e1a70, e1b_steps[s])
    ...
    ordered_e1b = [e1b_steps[s] for s in sorted(e1b_steps)]
    with open(os.path.join(out, "comparison.md"), "w") as fh:
        fh.write("# E1-B evaluation comparison (identical protocol: 7 benchmarks, greedy, 51,201 samples)\n\n")
    ...
    delta_md(result.get("vs_gap_step120"), "E1-B step120 vs GAP baseline (step120)",
             os.path.join(out, "vs_gap_step120.md"))
    delta_md(result.get("vs_e1a_step70"), "E1-B step120 vs E1-A step70 (E1-B start)",
             os.path.join(out, "vs_e1a_step70.md"))
    delta_md(result.get("vs_e10_step70"), "E1-B step120 vs E1-0 step70 (matched original-reward baseline)",
             os.path.join(out, "vs_e10_step70.md"))

    with open(os.path.join(out, "per_benchmark_deltas.md"), "w") as fh:
        fh.write("# E1-B step120 per-benchmark behavior, vs GAP step120 and E1-A step70\n\n")
        for bench in BENCH_ORDER:
            fh.write(f"## {bench}\n\n")
            fh.write("| metric | GAP step120 | E1-A step70 | E1-B step120 | d(E1B-GAP120) | d(E1B-E1A70) |\n")
            fh.write("|---|---|---|---|---|---|\n")
            row = {}
            for key, ref in (("gap", gap120), ("e1a", e1a70), ("e1b", e1b_steps.get(120))):
            ...
            for m in ["em"] + BEHAV:
                if m == "em":
                    vals = {k: (runs.get(v, {}).get("per_benchmark", {}) or {}).get(bench) if v else None
                            for k, v in (("gap", gap120), ("e1a", e1a70), ("e1b", e1b_steps.get(120)))}
                else:
    ...
    if result.get("vs_gap_step120"):
        ...
        print("E1-B step120 vs GAP step120: macroEM delta",
    ...
    if result.get("vs_e1a_step70"):
        ...
        print("E1-B step120 vs E1-A step70: macroEM delta", fnum(b["macro_em"]["delta"]),
