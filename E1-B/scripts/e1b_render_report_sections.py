#!/usr/bin/env python3
"""Render E1-B/FINAL_REPORT.md sections 6-10 and 12 from the run's own artifacts.
...
Everything is computed from files produced by the run (no hand-entered numbers):
    ...
    E1-B/checkpoints/checkpoint_stats.json                   (section 7)
    E1-B/evaluations/*/eval_summary.json                     (section 8)
    E1-B/evaluations/comparison/comparison.json              (sections 9/10)
    E1-B/evaluations/comparison/paired_comparison_*.json     (sections 9/10)

Usage:
    python3 e1b_render_report_sections.py                 # writes report_generated_sections.md
    python3 e1b_render_report_sections.py --splice        # also splices into FINAL_REPORT.md
"""
import argparse
import glob
import json
...
def sec6(run_dir, out):
    ...
    out.append("| metric | E1-B (shaped) | baseline: E1-0 steps61–70 (original EM reward) |")
    out.append("|---|---|---|")
    ...
    if qp:
        ...
        out.append("### Safety checks (E1-B_task.md §7)\n")
        out.append("| # | check | E1-B | reference | verdict |")
        out.append("|---|---|---|---|---|")
        ...
        out.append(f"| 4 | zero-search-and-correct | {safety.get('zero_search', {}).get('e1b_zero_search_correct_rate')} | 0 expected | "
                   f"{'OK' if not safety.get('zero_search', {}).get('e1b_zero_search_correct_rate') else 'REVIEW'} |")
...
def sec7(out):
    st = jload(os.path.join(E1B_ROOT, "checkpoints", "checkpoint_stats.json"), {})
    ...
    out.append("Manifest per checkpoint (shard sizes, `data.pt`, merged-HF presence): "
               "`E1-B/checkpoints/global_step_<N>.md`, `E1-B/checkpoints/manifest.json`.\n")


def load_evals():
    runs = {}
    for p in sorted(glob.glob(os.path.join(E1B_ROOT, "evaluations", "*", "eval_summary.json"))):
        with open(p) as fh:
            s = json.load(fh)
...
def sec8(out, runs):
    ...
    steps = set(step_of)
    out.append(f"E1-B checkpoints evaluated: {sorted(steps)}\n")
    out.append("| benchmark | " + " | ".join(f"E1-B step{s}" for s in sorted(steps)) + " |")
    ...
    out.append("")
    out.append("Flat artifacts required by `E1-B_task.md` §6: `E1-B/evaluations/results.json`, "
               "`benchmark_scores.csv`, `behavior_metrics.csv`, `overall_metrics.csv`.\n")


def sec9_10(out, runs):
    comp = jload(os.path.join(E1B_ROOT, "evaluations", "comparison", "comparison.json"), {})
    if not comp:
    ...
    def block(title, key, extra_note=""):
        ...
        out.append("| metric | baseline | E1-B | delta | delta % |")
        out.append("|---|---|---|---|---|")
        ...
        out.append("| benchmark | baseline | E1-B | delta |")
        out.append("|---|---|---|---|")
        for bench in BENCH_ORDER:
    ...
    # paired comparisons
    paired = {}
    for p in glob.glob(os.path.join(E1B_ROOT, "evaluations", "comparison", "paired_comparison_*.json")):
        with open(p) as fh:
            pj = json.load(fh)
    ...
    def paired_md(label, title):
        ...
        out.append(f"- search rounds: {o['rounds_baseline_mean']:.4f} → {o['rounds_e1b_mean']:.4f} "
        ...
        out.append(f"- EM: {o['em_baseline_mean']:.4f} → {o['em_e1b_mean']:.4f} ({o['em_delta']:+.4f}); "
                   f"up {o['em_up_prompts']:,} / down {o['em_down_prompts']:,}; sign test "
                   f"**p = {o['sign_test_p_em']:.3g}**")
        out.append("")
        out.append("| benchmark | n | rounds base | rounds E1-B | Δ | decreased | increased | ΔEM |")
        out.append("|---|---|---|---|---|---|---|---|")
        for ds, v in pj["per_benchmark"].items():
            out.append(f"| {ds} | {v['n_paired_prompts']:,} | {v['rounds_baseline_mean']:.3f} | "
                       f"{v['rounds_e1b_mean']:.3f} | {v['rounds_delta']:+.3f} | "
    ...
    block("## 9. 与 GAP baseline 比较 (vs GAP baseline)\n", "vs_gap_step120",
          ...
          "original EM reward) is the primary comparison; the E1-B branch starts from GAP step60 "
          "(via E1-A step70), so a GAP step60 comparison is also given in `comparison.json`.")
    paired_md("e1b_step120_vs_gap_step120", "Paired per-prompt test, E1-B step120 vs GAP step120:\n")
    block("## 10. 与 E1-A 比较 (vs E1-A)\n", "vs_e1a_step70",
          "E1-A step70 is the E1-B starting checkpoint, so this comparison measures what the "
          "**additional 50 shaped-reward steps** changed (within the same reward regime).")
    paired_md("e1b_step120_vs_e1a_step70", "Paired per-prompt test, E1-B step120 vs E1-A step70:\n")

    traj = comp.get("trajectory_from_e1a_step70") or {}
    if traj:
        out.append("### E1-B trajectory anchored at its own start (E1-A step70)\n")
...
def sec12(out, runs):
    ...
    # E1-A's stated success / stop criteria for E1-B
    ...
    out.append(f"| E1-A success criterion (rounds −1.5 % or more vs the E1-B start) | {'MET' if ok_rounds else 'NOT MET'} |")
    out.append("")
...
def main():
    ...
    args = ap.parse_args()
    run_dir = args.run_dir or open(os.path.join(E1B_ROOT, "latest_run.txt")).read().strip()
    ...
    sec12(out, runs)
    text = "\n".join(out)
    gen = os.path.join(E1B_ROOT, "report_generated_sections.md")
    with open(gen, "w") as fh:
        fh.write(text)
    print(f"wrote {gen} ({len(text)} chars)")

    if args.splice:
        rp = os.path.join(E1B_ROOT, "FINAL_REPORT.md")
        cur = open(rp).read()
        idx = cur.find("## 6. 训练过程")
        if idx < 0:
