"""E1-B reward manager: shaped reward + accuracy separation + diagnostics.

Registered name: `e1b_batch_shaped` (subclass of the ORIGINAL
...
1. For TRAINING it computes the shaped reward via
   `e1b_scorer.compute_score_em_efficiency_batch` (R = A*(1+0.05*E), cost =
...
2. `data.batch["acc"]` is restored to the ORIGINAL EM after the base class
   ...
   (E1-B_task.md "batch.py 要求").  `token_level_scores`/`token_level_rewards`
   keep the shaped reward, which is what GRPO and the group filter consume.
3. Passive diagnostics are appended to `<E1B_RUN_DIR>/diagnostics/`.
...
import e1b_cost
import e1b_state
import e1b_trainer_patch

E1B_RUN_DIR = os.environ.get("E1B_RUN_DIR")
E1B_START_GLOBAL_STEP = int(os.environ.get("E1B_START_GLOBAL_STEP", "60"))


@register("e1b_batch_shaped")
class E1BBatchRewardManager(BatchRewardManager):
    ...
    def __init__(self, tokenizer, num_examine, compute_score, reward_fn_key="data_source", **reward_kwargs):
        ...
        self.run_dir = E1B_RUN_DIR
        self.pid = os.getpid()
        self._last_step = None
        self._call_within_step = 0
        if self.is_train and self.run_dir:
            e1b_trainer_patch.ensure_driver_patch()
    ...
    def _next_call_index(self):
        with E1BBatchRewardManager._lock:
            idx = E1BBatchRewardManager._call_index["train"]
            E1BBatchRewardManager._call_index["train"] += 1
            return idx
    ...
    def verify(self, data):
        ...
        cost_info = e1b_cost.costs_from_batch(data.non_tensor_batch, n)
    ...
    def __call__(self, data, return_dict=False):
        ...
        if self.is_train:
            # ---- E1-B requirement: acc = original EM, reward = shaped ----
            try:
    ...
    def _diagnostics(self, data, result):
        """Passive diagnostics required by E1-B_task.md: reward / efficiency
        ...
        gstep = e1b_state.current_global_step()
