"""E1-B reward scorer: success-conditioned group-relative efficiency reward.
...
Design constraints (E1-B_task.md):
...
def _load_original_scorer():
    spec = importlib.util.spec_from_file_location("e1b_original_mhqa_train", _ORIGINAL_MHQA_TRAIN)
...
def get_lambda():
    try:
        return float(os.environ.get("E1B_LAMBDA", DEFAULT_LAMBDA))
...
def compute_score_em_efficiency_batch(
    ...
    **kwargs,
    ...
    for i in range(n):
        ...
        # E1-B invariant: the original scorer's score is the EM (no shaping there)
