...
pattern as e1b_checkpoint_patch.py.
...
`_ensure_monkey_patched()`, called from `_e1b_patched_init_distributed_env`,
...
def _ensure_monkey_patched():
    ...
    `_e1b_patched_init_distributed_env`, which runs inside that process (and,
...
def _e1b_patched_init_distributed_env(self, device_mesh_cpu, **kwargs):
...
SGLangRollout._init_distributed_env = _e1b_patched_init_distributed_env
