#!/usr/bin/env python3
"""E1-B CPU-only self-test for the shaped reward + accuracy separation.
...
Usage:  E1B_RUN_DIR=/tmp/e1b_selftest python3 e1b_selftest.py
"""
import json
import os
import shutil
...
RUN_DIR = os.environ.get("E1B_RUN_DIR", "/tmp/e1b_selftest")
...
def main():
    ...
    os.environ["E1B_RUN_DIR"] = RUN_DIR

    import e1b_cost
    import e1b_scorer
    import e1b_state
    from e1b_reward_manager import E1BBatchRewardManager
    ...
    if not callable(getattr(e1b_scorer, "compute_score_em_batch", None)):
        failures.append("compute_score_em_batch missing from e1b_scorer")
    ...
    scored = e1b_scorer.compute_score_em_efficiency_batch(
    ...
    tm = E1BBatchRewardManager(tok, num_examine=0, compute_score=e1b_scorer.compute_score_em_efficiency_batch)
    e1b_state.TRAINER = types.SimpleNamespace(global_steps=61)
    ...
    vm = E1BBatchRewardManager(tok2, num_examine=1, compute_score=e1b_scorer.compute_score_em_batch)
