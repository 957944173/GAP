#!/usr/bin/env python3
"""E1-E CPU-only self-test for the shaped reward + accuracy separation.
...
Usage:  E1E_RUN_DIR=/tmp/e1e_selftest python3 e1e_selftest.py
"""
import json
import os
import shutil
...
RUN_DIR = os.environ.get("E1E_RUN_DIR", "/tmp/e1e_selftest")
...
def main():
    ...
    os.environ["E1E_RUN_DIR"] = RUN_DIR

    import e1e_cost
    import e1e_scorer
    import e1e_state
    from e1e_reward_manager import E1EEatchRewardManager
    ...
    if not callable(getattr(e1e_scorer, "compute_score_em_batch", None)):
        failures.append("compute_score_em_batch missing from e1e_scorer")
    ...
    scored = e1e_scorer.compute_score_em_efficiency_batch(
    ...
    tm = E1EEatchRewardManager(tok, num_examine=0, compute_score=e1e_scorer.compute_score_em_efficiency_batch)
    e1e_state.TRAINER = types.SimpleNamespace(global_steps=61)
    ...
    vm = E1EEatchRewardManager(tok2, num_examine=1, compute_score=e1e_scorer.compute_score_em_batch)
