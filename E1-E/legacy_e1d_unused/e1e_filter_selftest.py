#!/usr/bin/env python3
"""E1-E preflight offline selftest (E1-E_task.md §9) — CPU only, no GPU, no server.

Runs the seven mandatory tests on REAL saved rollouts and writes
`E1-E/analysis/preflight_selftest.{md,json}`.

  TEST 1  reward equivalence: E1-E scorer == E1-B scorer, rollout by rollout, on the real
...
E1E_SCORER = os.path.join(E1D, "code/e1e_scorer.py")
E1E_MANAGER = os.path.join(E1D, "code/e1e_reward_manager.py")
E1E_TRAIN_LAUNCHER = None  # set from argv if given
...
def main():
    ...
    mod_d = load_module(E1E_SCORER, "e1e_scorer_selftest")

    # ---------------- TEST 2: lambda -------------------------------------
    lam_b = float(mod_b.DEFAULT_LAMBDA)
    lam_d = float(mod_d.DEFAULT_LAMBDA)
    lam_src = float(os.environ.get("E1E_LAMBDA", "0.05"))
    tests["TEST2_lambda"] = {
        "pass": abs(lam_b - LAMBDA_EXPECTED) < 1e-12 and abs(lam_d - LAMBDA_EXPECTED) < 1e-12
                and lam_src == LAMBDA_EXPECTED,
        "e1b_default_lambda": lam_b, "e1e_default_lambda": lam_d,
        "e1e_env_lambda_default": lam_src, "expected": LAMBDA_EXPECTED,
    }
    ...
    result = {"all_pass": all_pass, "tests": tests,
              "scorer_sha256": {
                  ...
                  "e1d": hashlib.sha256(open(E1E_SCORER, "rb").read()).hexdigest()},
              "target_train_batch_size": TARGET_TRAIN_BATCH_SIZE}
    os.makedirs(os.path.dirname(OUT_JSON), exist_ok=True)
    with open(OUT_JSON, "w") as fh:
        json.dump(result, fh, indent=2, default=str)

    md = ["# E1-E preflight offline selftest (E1-E_task.md §9)", "",
