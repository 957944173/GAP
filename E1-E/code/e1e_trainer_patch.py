...
Loaded lazily by e1e_reward_manager.E10BatchAuditRewardManager.__init__ (which
...
What is installed (all pure observation, no control-flow/reward/sampling change):
  1. RayPPOTrainer.__init__ wrapper -> registers the live trainer instance in
     e1e_state so the audit can read the true `global_steps`.
...
def ensure_driver_patch():
    ...
    def _e1e_init(self, *args, **kwargs):
        result = orig_init(self, *args, **kwargs)
        e1e_state.TRAINER = self
        try:
            e1e_state.diag(
    ...
    trainer_cls.__init__ = _e1e_init

    orig_load = trainer_cls._load_checkpoint

    @functools.wraps(orig_load)
    def _e1e_load_checkpoint(self, *args, **kwargs):
        e1e_state.diag("_load_checkpoint: entered (driver)")
    ...
    trainer_cls._load_checkpoint = _e1e_load_checkpoint

    orig_validate = trainer_cls._validate

    @functools.wraps(orig_validate)
    def _e1e_validate(self, *args, **kwargs):
        e1e_state.diag(f"_validate: entered (global_steps={getattr(self, 'global_steps', None)})")
        result = orig_validate(self, *args, **kwargs)
        e1e_state.diag(f"_validate: returned (global_steps={getattr(self, 'global_steps', None)})")
        return result

    trainer_cls._validate = _e1e_validate

    _PATCHED = True
    e1e_state.diag("driver-side passive instrumentation installed")
