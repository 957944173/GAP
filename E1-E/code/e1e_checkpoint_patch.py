...
def _e1e_diag(self, msg):
    ...
    print(f"[E1-E ckpt-diag][rank {self.rank}] {msg} (t={time.time():.1f})", flush=True)


def _e1e_patched_load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
    ...
    _e1e_diag(self, "load_checkpoint: entered")
    ...
    with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
        _e1e_diag(self, "entered get_fsdp_state_ctx")
        if self.should_load_model:
            ...
            _e1e_diag(self, f"torch.load(model) starting: {local_model_path}")
            model_state_dict = torch.load(local_model_path, weights_only=False, map_location="cpu")
            _e1e_diag(self, "torch.load(model) done; calling model.load_state_dict()")
            self.model.load_state_dict(model_state_dict)
            _e1e_diag(self, "model.load_state_dict() returned")
        ...
        if self.should_load_optimizer:
            ...
            _e1e_diag(self, f"torch.load(optim) starting: {local_optim_path}")
            optimizer_state_dict = torch.load(local_optim_path, weights_only=False, map_location="cpu")
            _e1e_diag(self, "torch.load(optim) done; calling optimizer.load_state_dict()")
            self.optimizer.load_state_dict(optimizer_state_dict)
            _e1e_diag(self, "optimizer.load_state_dict() returned")
            log_with_rank(f"Loaded optimizer from {remote_optim_path}", rank=self.rank, logger=logger)
    _e1e_diag(self, "exited get_fsdp_state_ctx")
    ...
    _e1e_diag(self, "about to enter final torch.distributed.barrier()")
    torch.distributed.barrier()
    _e1e_diag(self, "load_checkpoint: fully returned")


FSDPCheckpointManager.load_checkpoint = _e1e_patched_load_checkpoint
