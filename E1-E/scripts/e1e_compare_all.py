#!/usr/bin/env python3
"""Multi-way comparison of all E1-E / reference evaluations (E1-E_task.md 8/9/10).

Reads every `eval_summary.json` under E1-E/evaluations/ (E1-E step80..120 plus the
re-aggregated references gap_step60, gap_step120, gap_step180, e1a_step70, e10_step70)
and emits:

    E1-E/evaluations/comparison/comparison.json
    E1-E/evaluations/comparison/comparison.md            (full table, all runs)
    E1-E/evaluations/comparison/vs_gap_step120.md        (E1-E step120 vs GAP baseline)
    E1-E/evaluations/comparison/vs_e1a_step70.md         (E1-E step120 vs E1-A start)
    E1-E/evaluations/comparison/per_benchmark_deltas.md  (per-benchmark behaviour deltas)
...
def main():
    ...
    e1b120 = pick("e1b_step120_reference")
    e1d70 = pick("e1d_step70_reference")
    e1d120 = pick("e1d_step120_reference")
    # E1-E's OWN checkpoints only: exactly `step<N>_<YYYYmmdd>_<HHMMSS>`; reference symlinks
    # carry explicit `*_reference` names and can never be mistaken for the branch's steps.
    step_re = re.compile(r"^step(\d+)_\d{8}_\d{6}$")
    e1e_steps = {int(m.group(1)): k for k in runs for m in [step_re.match(k)] if m}

    result = {"runs": runs, "roles": {"gap_step120": gap120, "gap_step60": gap60,
                                      "gap_step180": gap180, "e1a_step70": e1a70,
                                      "e10_step70": e10_70, "e1b_step120": e1b120,
                                      "e1d_step70": e1d70, "e1d_step120": e1d120,
                                      "e1e_steps": {str(k): v for k, v in sorted(e1e_steps.items())}}}
    ...
    result["vs_gap_step120"] = delta_block(gap120, e1e_steps.get(120))
    result["vs_e1b_step120"] = delta_block(e1b120, e1e_steps.get(120))
    result["vs_e1d_step120"] = delta_block(e1d120, e1e_steps.get(120))
    result["vs_e1a_step70"] = delta_block(e1a70, e1e_steps.get(120))
    result["vs_gap_step60"] = delta_block(gap60, e1e_steps.get(120))
    result["vs_e10_step70"] = delta_block(e10_70, e1e_steps.get(120))
    result["vs_e1d_step70"] = delta_block(e1d70, e1e_steps.get(120))
    result["e1e120_vs_e1e70"] = delta_block(e1e_steps.get(70), e1e_steps.get(120))
    # trajectory of the E1-E branch, anchored at its own start (E1-A step70)
    traj = {}
    for s in sorted(e1e_steps):
        traj[s] = delta_block(e1a70, e1e_steps[s])
    ...
    ordered_refs = [gap60, gap120, gap180, e10_70, e1a70, e1b120, e1d70, e1d120]
    ordered_e1b = [e1e_steps[s] for s in sorted(e1e_steps)]
    with open(os.path.join(out, "comparison.md"), "w") as fh:
        fh.write("# E1-E evaluation comparison (identical protocol: 7 benchmarks, greedy, 51,201 samples)\n\n")
    ...
    delta_md(result.get("vs_gap_step120"), "E1-E step120 vs GAP baseline (step120)",
             os.path.join(out, "vs_gap_step120.md"))
    delta_md(result.get("vs_e1b_step120"), "E1-E step120 vs E1-B step120",
             os.path.join(out, "vs_e1b_step120.md"))
    delta_md(result.get("vs_e1d_step120"), "E1-E step120 vs E1-D step120 (mandated four-way)",
             os.path.join(out, "vs_e1d_step120.md"))
    delta_md(result.get("e1e120_vs_e1e70"), "E1-E step120 vs E1-E step70 (own trajectory)",
             os.path.join(out, "e1e120_vs_e1e70.md"))
    delta_md(result.get("vs_e1a_step70"), "E1-E step120 vs E1-A step70 (E1-E start)",
             os.path.join(out, "vs_e1a_step70.md"))
    delta_md(result.get("vs_e10_step70"), "E1-E step120 vs E1-0 step70 (matched original-reward baseline)",
             os.path.join(out, "vs_e10_step70.md"))

    with open(os.path.join(out, "per_benchmark_deltas.md"), "w") as fh:
        fh.write("# E1-E step120 per-benchmark behavior, vs GAP step120 and E1-A step70\n\n")
        for bench in BENCH_ORDER:
            fh.write(f"## {bench}\n\n")
            fh.write("| metric | GAP step120 | E1-A step70 | E1-E step120 | d(E1B-GAP120) | d(E1B-E1A70) |\n")
            fh.write("|---|---|---|---|---|---|\n")
            row = {}
            for key, ref in (("gap", gap120), ("e1a", e1a70), ("e1b", e1e_steps.get(120))):
            ...
            for m in ["em"] + BEHAV:
                if m == "em":
                    vals = {k: (runs.get(v, {}).get("per_benchmark", {}) or {}).get(bench) if v else None
                            for k, v in (("gap", gap120), ("e1a", e1a70), ("e1b", e1e_steps.get(120)))}
                else:
    ...
    if result.get("vs_gap_step120"):
        ...
        print("E1-E step120 vs GAP step120: macroEM delta",
    ...
    if result.get("vs_e1a_step70"):
        ...
        print("E1-E step120 vs E1-A step70: macroEM delta", fnum(b["macro_em"]["delta"]),
              "rounds delta", fnum(b["behavior"]["retrieval_rounds_mean"]["delta"], 4),
              f"({fnum(b['behavior']['retrieval_rounds_mean']['pct'], 2)}%)")
    # the mandated four-way view (GAP120 / E1-B120 / E1-D120 / E1-E120)
    four = {"gap_step120": gap120, "e1b_step120": e1b120, "e1d_step120": e1d120,
            "e1e_step120": e1e_steps.get(120)}
    rows = []
    for role, key in four.items():
        r = runs.get(key) if key else None
        rows.append({"role": role, "run": key,
                     "micro_em": (r or {}).get("micro_em"),
                     "macro_em": (r or {}).get("macro_em"),
                     "rounds": ((r or {}).get("overall") or {}).get("retrieval_rounds_mean"),
                     "queries": ((r or {}).get("overall") or {}).get("search_queries_mean"),
                     "tokens": ((r or {}).get("overall") or {}).get("response_tokens_mean"),
                     "zero_search": ((r or {}).get("overall") or {}).get("zero_search_rate")})
    result["four_way_step120"] = rows
    with open(os.path.join(out, "four_way_step120.md"), "w") as fh:
        fh.write("# Four-way comparison at step120 (identical 7-benchmark protocol)\n\n")
        fh.write("| run | micro EM | macro EM | rounds | queries | tokens | zero-search |\n")
        fh.write("|---|---|---|---|---|---|---|\n")
        for r in rows:
            fh.write("| `{}` | {} | {} | {} | {} | {} | {} |\n".format(
                r["run"], fnum(r["micro_em"], 6), fnum(r["macro_em"], 6), fnum(r["rounds"], 4),
                fnum(r["queries"], 4), fnum(r["tokens"], 2), fnum(r["zero_search"], 6)))
        fh.write("\n")
    with open(os.path.join(out, "comparison.json"), "w") as fh:
        json.dump(result, fh, indent=2, default=str)
    print(json.dumps({"four_way_step120": rows}, indent=2, default=str))
    return 0
