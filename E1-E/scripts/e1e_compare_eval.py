#!/usr/bin/env python3
"""E1-E vs baseline 7-benchmark comparison.

Usage:
    python3 e1e_compare_eval.py --baseline <dir with eval_summary.json> \
...
def main():
    ...
    comparison = {"baseline_label": b["label"], "e1e_label": a["label"],
    ...
    if em_a and em_b:
        comparison["macro_em_e1e_recomputed"] = sum(em_a) / len(em_a)
        comparison["macro_em_baseline_recomputed"] = sum(em_b) / len(em_b)

    with open(os.path.join(args.out, "comparison.json"), "w") as f:
        json.dump(comparison, f, indent=2, default=str)

    lines = ["# E1-E vs baseline (GAP step60->70 original reward): 7 benchmarks", "",
             f"- E1-E (shaped reward): `{a['label']}`",
             f"- baseline (original EM): `{b['label']}`", "",
             "## EM", "", "| benchmark | E1-E EM | baseline EM | delta |", "|---|---|---|---|"]
    for ds in ds_list:
