#!/usr/bin/env bash
# Shared E1-D evaluation helper: reuses the ORIGINAL GAP evaluation implementation
...
e1d_run_eval() {
    ...
    if [[ ! -s "$merged/config.json" ]]; then
        ...
        echo "[E1-D eval:$label] merge already present: $merged"
    fi
    if [[ ! -s "$merged/config.json" ]]; then
        echo "[E1-D eval:$label] ERROR: merge failed" >&2; return 1
    fi
    # shellcheck disable=SC1090,SC1091
    source "$repo_root/Agent/evaluation/mhqa_agent/eval_mhqa_agent_4gpu_common.sh"
    export EVAL_MODEL_KIND="e1d"
    ...
    python3 "$e1d_root/scripts/e1d_analyze_eval.py" --label "$label" --log "$eval_log" --dump "$dump" --out "$out_dir"
    python3 "$e1d_root/scripts/e1d_collect_evaluation_artifacts.py" --eval-root "$e1d_root/eval_results" --out "$e1d_root/eval_results" || true
}
