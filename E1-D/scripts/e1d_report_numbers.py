#!/usr/bin/env python3
"""Print every number needed to write E1-D/FINAL_REPORT.md, from the artifacts on disk.

Usage: python3 E1-D/scripts/e1d_report_numbers.py [--comparison <dir>]
...
def main():
    ...
    for step in (70, 120):
        ...
        show(f"  filter_decoupling step{step}", integ.get("filter_decoupling"))
    ...
    if comp_dir:
        ...
        if c:
            ...
            for key in ("vs_gap_step120", "vs_e1a_step70", "vs_e10_step70", "vs_gap_step60"):
                b = c.get(key)
                if not b:
                    continue
        ...
        for p in sorted(glob.glob(os.path.join(comp_dir, "paired_comparison_*.json"))):
            ...
            print(f"  rounds {o['rounds_baseline_mean']:.4f} -> {o['rounds_e1d_mean']:.4f} "
            ...
            print(f"  EM     {o['em_baseline_mean']:.4f} -> {o['em_e1d_mean']:.4f} ({o['em_delta']:+.4f}) "
    ...
    for tag in ("D1", "D2"):
        for base in ("vs_E1_0", "vs_E1_B"):
            ...
            for k in ("n_rollouts", "n_groups", "n_calls", "em_rate", "reward_mean", "efficiency_mean",
                      ...
                      "filter_retention_shaped", "filter_newly_added", "efficiency_active_groups",
                      "efficiency_supervision_ratio"):
                print(f"  {k:34} E1-D={e.get(k)}  baseline={b.get(k)}")
