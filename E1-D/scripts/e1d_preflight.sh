#!/usr/bin/env bash
# =====================================================================================
# E1-D preflight (E1-D_task.md §5, §6, §7).
#   1. Wiki retriever: probe the URL the repo tool config uses; REUSE if healthy,
#      otherwise start it the way the repo/E1-A/E1-B did.  Logged to E1-D/logs/.
...
E1D_EXP_NAME="${E1D_EXP_NAME:-DAPO-GAP3B-MHQA-Agent-E1D-step60to120-4gpu}"

mkdir -p "$E1D_ROOT"/{code,scripts,configs,logs,diagnostics,analysis,eval_results,manifests,runs,checkpoints}
WIKI_LOG="$E1D_ROOT/logs/wiki_service.log"
LOG="$E1D_ROOT/logs/preflight.log"
exec > >(tee -a "$LOG") 2>&1

echo "===== E1-D preflight $(date -Is) ====="
...
echo "(E1-D kills no unknown process)"
...
for p in "$REPO_ROOT/Agent/data/mhqa_agent/GAP-MHQA-RL-Dataset/GAP-RL-16w.cleaned.parquet" \
         ...
         "$E1D_ROOT/code/e1d_scorer.py" "$E1D_ROOT/code/e1d_reward_manager.py" \
         "$E1D_ROOT/code/pythonpath_e1d/sitecustomize.py" "$E1D_ROOT/scripts/run_e1d_train.sh"; do
...
if [[ "$CKPT_OK" != "1" ]]; then
    echo "FATAL: missing inputs; E1-D_task.md §7 forbids substituting them. STOPPING." >&2
    echo "STATUS=BLOCKED_INPUTS" > "$E1D_ROOT/logs/preflight_status.txt"; exit 3
fi
for d in "$REPO_ROOT/experiments/$E1D_EXP_NAME" "$E1D_ROOT/runs/latest_D1.txt" "$E1D_ROOT/eval_results/latest_step70.txt"; do
    if [[ -e "$d" ]]; then echo "EXISTS $d (launchers never overwrite; D2 may continue its own dir)"; else echo "ABSENT $d"; fi
done

# ---------------------------------------------------------------- provenance pre
python3 - "$REPO_ROOT" "$E1D_ROOT/manifests/original_files_manifest_pre.json" <<'PY'
...
} > "$E1D_ROOT/logs/preflight_status.txt"
cat "$E1D_ROOT/logs/preflight_status.txt"
echo "===== E1-D preflight DONE (STATUS=OK) ====="
