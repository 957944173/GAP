#!/usr/bin/env bash
# E1-D integrity re-verification: the original GAP files must be byte-identical before
# and after the whole E1-D experiment (E1-D_task.md "不要直接修改原 GAP 主代码").
#
# Recomputes the manifest written by scripts/e1d_preflight.sh and diffs it.
# Outputs: E1-D/configs/original_files_manifest_post.txt
#          E1-D/configs/original_files_diff.txt  (empty == unchanged)
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
E1D_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$E1D_ROOT/.." && pwd)"
OUT="$E1D_ROOT/configs"
mkdir -p "$OUT"
...
lines = ["# E1-D preflight original-file manifest (sha256)"]
for rel in files:
