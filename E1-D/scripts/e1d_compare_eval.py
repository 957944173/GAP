#!/usr/bin/env python3
"""E1-D vs baseline 7-benchmark comparison.

Usage:
    python3 e1d_compare_eval.py --baseline <dir with eval_summary.json> \
                                --e1d <dir with eval_summary.json> --out <dir>
...
def main():
    ...
    ap.add_argument("--e1d", required=True)
    ...
    comparison = {"baseline_label": b["label"], "e1d_label": a["label"],
    ...
    for m in ["macro_em", "micro_em", "n_samples", "retrieval_rounds_mean", "search_queries_mean",
        ...
        comparison["overall"][m] = {"e1b": av, "baseline": bv,
                                    "delta": (av - bv) if isinstance(av, (int, float)) and isinstance(bv, (int, float)) else None}
    ...
    if em_a and em_b:
        comparison["macro_em_e1d_recomputed"] = sum(em_a) / len(em_a)
        comparison["macro_em_baseline_recomputed"] = sum(em_b) / len(em_b)

    with open(os.path.join(args.out, "comparison.json"), "w") as f:
        json.dump(comparison, f, indent=2, default=str)

    lines = ["# E1-D vs baseline (GAP step60->70 original reward): 7 benchmarks", "",
             f"- E1-D (shaped reward): `{a['label']}`",
             f"- baseline (original EM): `{b['label']}`", "",
             "## EM", "", "| benchmark | E1-D EM | baseline EM | delta |", "|---|---|---|---|"]
    for ds in ds_list:
