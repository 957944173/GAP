#!/usr/bin/env python3
"""Paired per-prompt comparison of two eval dumps (E1-D vs matched baseline).
...
def main():
    ...
    ap.add_argument("--e1d-dump", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--label", default="e1d_vs_baseline")
    args = ap.parse_args()
    os.makedirs(args.out, exist_ok=True)

    b = load(args.baseline_dump)
    a = load(args.e1d_dump)
    ...
    # (e.g. E1-D step120 vs GAP step120 AND vs E1-A step70) without overwriting
    for name in (f"paired_comparison_{args.label}.json", "paired_comparison.json"):
        with open(os.path.join(args.out, name), "w") as f:
            json.dump(out, f, indent=2, default=str)

    o = out["overall"]
    lines = ["# Paired per-prompt comparison (E1-D vs matched baseline)", "",
             f"- paired prompts: **{o['n_paired_prompts']:,}**", "",
             "| metric | E1-D | baseline | delta |", "|---|---|---|---|",
             f"| search rounds | {o['rounds_e1d_mean']:.4f} | {o['rounds_baseline_mean']:.4f} | "
             f"{o['rounds_delta']:+.4f} ({(o['rounds_delta']/max(o['rounds_baseline_mean'],1e-9))*100:+.2f}%) |",
             f"| search queries | {o['queries_e1d_mean']:.4f} | {o['queries_baseline_mean']:.4f} | "
             f"{o['queries_delta']:+.4f} |",
             f"| response chars | {o['chars_e1d_mean']:.1f} | {o['chars_baseline_mean']:.1f} | "
             f"{o['chars_e1d_mean']-o['chars_baseline_mean']:+.1f} |",
             f"| EM | {o['em_e1d_mean']:.4f} | {o['em_baseline_mean']:.4f} | {o['em_delta']:+.4f} |",
             "",
    ...
    for ds, v in out["per_benchmark"].items():
        lines.append(f"| {ds} | {v['n_paired_prompts']:,} | {v['rounds_e1d_mean']:.3f} | "
