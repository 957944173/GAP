#!/usr/bin/env python3
"""E1-D training-side diagnostics + comparison against the E1-0 baseline run.

Reads the diagnostics written by the E1-D reward manager
(`<E1D_RUN_DIR>/diagnostics/{rollouts,groups,calls}_pid*.jsonl`) and, for the
...
Outputs (into --out):
    training_summary.json               total/overall metrics for E1-D (+baseline)
    ...
    training_comparison.json/md         E1-D vs E1-0 per-step and overall
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--run-dir", required=True, help="E1-D run dir containing diagnostics/")
    ...
    if not rolls:
        raise SystemExit(f"no E1-D diagnostics found under {args.run_dir}/diagnostics")

    e1d = summarise(rolls, groups, "E1-D: shaped reward + EM filter")
    ...
    reward_dist = {
        "e1d_reward_hist": e1d["reward_hist"],
        "e1d_em_rate": e1d["em_rate"],
        "e1d_reward_nonzero_rate": e1d["reward_nonzero_rate"],
    ...
    eff_dist = {
        "e1d_efficiency_hist": e1d["efficiency_hist"],
        "e1d_efficiency_mean": e1d["efficiency_mean"],
        "e1d_efficiency_nonzero_rate": e1d["efficiency_nonzero_rate"],
        "e1d_efficiency_active_groups": e1d.get("efficiency_active_groups"),
    }
    search_behavior = {
        ...
        "delta_e1d_minus_baseline": ({k: (e1d[k] - baseline[k]) for k in
    ...
    group_cc = {"e1d_group_correct_count_hist": e1d.get("group_correct_count_hist"),
                "e1d_group_correct_count_mean": e1d.get("group_correct_count_mean"),
                "baseline_group_correct_count_hist": (baseline or {}).get("group_correct_count_hist")}
    retention = {
        "e1d_baseline_kept_groups": e1d.get("filter_retention_baseline"),
        "e1d_shaped_kept_groups": e1d.get("filter_retention_shaped"),
        "e1d_newly_added_groups": e1d.get("filter_newly_added"),
        "e1d_efficiency_active_groups": e1d.get("efficiency_active_groups"),
        "e1d_efficiency_supervision_ratio": e1d.get("efficiency_supervision_ratio"),
    ...
    safety = {
        ...
        "zero_search": {
            "e1d_zero_search_rate": e1d.get("zero_search_rate"),
            "baseline_zero_search_rate": (baseline or {}).get("zero_search_rate"),
            "e1d_zero_search_correct_rate": (
    ...
    summary = {"e1d": e1d, "baseline": baseline,
               "e1d_per_step": per_step_table(rolls, groups),
               "e1d_calls": calls}
    for name, obj in (("training_summary.json", summary),
                      ...
                      ("training_comparison.json", {"e1d": e1d, "baseline": baseline,
                                                     "per_step_e1d": summary["e1d_per_step"]})):
    ...
    lines = ["# E1-D training-side diagnostics (step 60 -> 70)", ""]
    lines.append("| metric | E1-D (shaped) | baseline E1-0 (original EM) | delta |")
    lines.append("|---|---|---|---|")
