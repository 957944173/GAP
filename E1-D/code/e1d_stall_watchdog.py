...
handler installed by code/e1d_hang_diagnostics.py in every process that runs
with PYTHONPATH=<code>/pythonpath_e10 and E1D_RUN_DIR set: delivering SIGUSR1
...
e1d_hang_diagnostics, i.e. it has a SIGUSR1 handler and will not be killed by
the signal.  Stale files from previous attempts are snapshotted and ignored, so
a recycled pid can never be signalled.

Usage:
    python3 e1d_stall_watchdog.py --log <launcher log> --run-dir <RUN_DIR> \
