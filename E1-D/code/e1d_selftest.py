#!/usr/bin/env python3
"""E1-D CPU-only self-test for the shaped reward + accuracy separation.
...
Usage:  E1D_RUN_DIR=/tmp/e1d_selftest python3 e1d_selftest.py
"""
import json
import os
import shutil
...
RUN_DIR = os.environ.get("E1D_RUN_DIR", "/tmp/e1d_selftest")
...
def main():
    ...
    os.environ["E1D_RUN_DIR"] = RUN_DIR

    import e1d_cost
    import e1d_scorer
    import e1d_state
    from e1d_reward_manager import E1DDatchRewardManager
    ...
    if not callable(getattr(e1d_scorer, "compute_score_em_batch", None)):
        failures.append("compute_score_em_batch missing from e1d_scorer")
    ...
    scored = e1d_scorer.compute_score_em_efficiency_batch(
    ...
    tm = E1DDatchRewardManager(tok, num_examine=0, compute_score=e1d_scorer.compute_score_em_efficiency_batch)
    e1d_state.TRAINER = types.SimpleNamespace(global_steps=61)
    ...
    vm = E1DDatchRewardManager(tok2, num_examine=1, compute_score=e1d_scorer.compute_score_em_batch)
