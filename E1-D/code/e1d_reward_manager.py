"""E1-D reward manager: shaped reward + accuracy separation + diagnostics.

E1-D FILTER DECOUPLING
-----------------------
E1-D trains with the **E1-B shaped reward** but with the **original GAP filter**:
`algorithm.filter_groups.metric = em`.  The trainer therefore groups by
`new_batch.non_tensor_batch["em"]` (which this manager publishes through
`reward_extra_info -> reward_extra_infos_dict`, ray_trainer.py:1755) while
`token_level_rewards` still carries the shaped reward into GRPO.  Nothing in this
manager decides the filter - it only records both the actual (EM) and the
counterfactual (shaped) retained sets.

Registered name: `e1d_batch_shaped` (subclass of the ORIGINAL
...
1. For TRAINING it computes the shaped reward via
   `e1d_scorer.compute_score_em_efficiency_batch` (R = A*(1+0.05*E), cost =
...
2. `data.batch["acc"]` is restored to the ORIGINAL EM after the base class
   ...
   (E1-D_task.md "batch.py 要求").  `token_level_scores`/`token_level_rewards`
   keep the shaped reward, which is what GRPO and the group filter consume.
3. Passive diagnostics are appended to `<E1D_RUN_DIR>/diagnostics/`.
...
import e1d_cost
import e1d_state
import e1d_trainer_patch

E1D_RUN_DIR = os.environ.get("E1D_RUN_DIR")
#: E1-D's only scientific change: the trainer's group filter uses the ORIGINAL EM
#: (`algorithm.filter_groups.metric=em`), not the shaped reward.
E1D_FILTER_METRIC = os.environ.get("E1D_FILTER_METRIC", "em")
E1D_START_GLOBAL_STEP = int(os.environ.get("E1D_START_GLOBAL_STEP", "60"))


@register("e1d_batch_shaped")
class E1DDatchRewardManager(BatchRewardManager):
    ...
    def __init__(self, tokenizer, num_examine, compute_score, reward_fn_key="data_source", **reward_kwargs):
        ...
        self.run_dir = E1D_RUN_DIR
        self.pid = os.getpid()
        self._last_step = None
        self._call_within_step = 0
        if self.is_train and self.run_dir:
            e1d_trainer_patch.ensure_driver_patch()
    ...
    def _next_call_index(self):
        with E1DDatchRewardManager._lock:
            idx = E1DDatchRewardManager._call_index["train"]
            E1DDatchRewardManager._call_index["train"] += 1
            return idx
    ...
    def verify(self, data):
        ...
        cost_info = e1d_cost.costs_from_batch(data.non_tensor_batch, n)
    ...
    def __call__(self, data, return_dict=False):
        ...
        if self.is_train:
            # ---- E1-D requirement: acc = original EM, reward = shaped ----
            try:
    ...
    def _diagnostics(self, data, result):
        """Passive diagnostics required by E1-D_task.md: reward / efficiency
        ...
        gstep = e1d_state.current_global_step()
        ...
        for uid, g in group_agg.items():
            ...
            group_records.append({
                ...
                # ---- E1-D filter-decoupling diagnostics -------------------------
                "filter_metric": E1D_FILTER_METRIC,
                "actual_filter_keep": baseline_keep,            # EM std>0 == what the trainer keeps
                "counterfactual_shaped_keep": shaped_keep,      # what E1-B's filter would have kept
                "mixed_correctness": bool(0 < int(em.sum()) < len(em)),
                "all_correct": bool(int(em.sum()) == len(em)),
                "all_wrong": bool(int(em.sum()) == 0),
                "efficiency_active_excluded_by_em_filter": bool(
                    np.max(g["eff"]) > 0.0 and not baseline_keep),
            })
        ...
        call_rec = {
            ...
            # ---- E1-D filter-decoupling diagnostics -------------------------
            "filter_metric": E1D_FILTER_METRIC,
            "actual_retained_groups": n_baseline_keep,
            "counterfactual_shaped_retained_groups": n_shaped_keep,
            "n_all_correct_kept": int(sum(1 for gr in group_records
                                          if gr["all_correct"] and gr["actual_filter_keep"])),
            "n_mixed_kept": int(sum(1 for gr in group_records
                                    if gr["mixed_correctness"] and gr["actual_filter_keep"])),
            "n_all_wrong_kept": int(sum(1 for gr in group_records
                                        if gr["all_wrong"] and gr["actual_filter_keep"])),
            "n_efficiency_active_excluded": int(sum(
                1 for gr in group_records if gr["efficiency_active_excluded_by_em_filter"])),
            "n_mixed_efficiency_active_kept": int(sum(
                1 for gr in group_records if gr["mixed_correctness"]
                and gr["actual_filter_keep"] and gr["efficiency_active"])),
            "efficiency_active_retained_actual": int(sum(
                1 for gr in group_records if gr["actual_filter_keep"] and gr["efficiency_active"])),
            "efficiency_active_retained_shaped_filter": int(sum(
                1 for gr in group_records if gr["counterfactual_shaped_keep"] and gr["efficiency_active"])),
            "efficiency_supervision_ratio_actual": (
                (sum(1 for gr in group_records if gr["mixed_correctness"]
                     and gr["actual_filter_keep"] and gr["efficiency_active"]) / n_baseline_keep)
                if n_baseline_keep else None),
        }
