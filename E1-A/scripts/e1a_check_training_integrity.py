#!/usr/bin/env python3
"""E1-0 post-training integrity check (E1-0_task.md section 18).

Verifies the step70 checkpoint, the step61..step70 training log records, and
...
Usage:
    E1A_RUN_DIR=<run dir> [E1A_EXPERIMENT_NAME=<name>] python3 e1a_check_training_integrity.py
"""
import json
import math
import os
...
RUN_DIR = (os.environ.get("E1A_RUN_DIR")
           or os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
EXPERIMENT_NAME = os.environ.get("E1A_EXPERIMENT_NAME", "DAPO-GAP3B-MHQA-Agent-E1A-step60to70-4gpu")
EXP_DIR = os.path.join(REPO, "experiments", EXPERIMENT_NAME)
CKPT = os.path.join(EXP_DIR, "global_step_70")
...
def main():
    ...
    if log["exists"]:
        ...
        log["steps_61_to_70_all_present"] = all(s in steps for s in range(61, 71))
        log["missing_steps_61_to_70"] = [s for s in range(61, 71) if s not in steps]
        log["max_step"] = max(steps) if steps else None
        log["start_step_line_present"] = "Setting global step to 60" in clean
    ...
    resume = {"step60_extra_state": None, "step70_extra_state": None}
    try:
        import torch

        for tag, d in (("step60_extra_state", os.path.join(REPO, "experiments",
                                                           "DAPO-GAP3B-MHQA-Agent-4gpu", "global_step_60", "actor")),
                       ("step70_extra_state", os.path.join(CKPT, "actor"))):
    ...
    state_resume_ok = (
        (resume.get("step60_extra_state") or {}).get("lr_scheduler_last_epoch") == 60
        and (resume.get("step70_extra_state") or {}).get("lr_scheduler_last_epoch") == 70
        and (resume.get("step70_extra_state") or {}).get("rng_present") is True
    )

    # ---------------- verdict ----------------
    ok = (
        ckpt.get("exists")
        and ckpt.get("global_step_from_path") == 70
        and all(v.get("exists") for v in ckpt["shards"].values())
        and log.get("steps_61_to_70_all_present")
    ...
    # ---------------- markdown ----------------
    lines.append("# E1-A post-training integrity check\n")
    ...
    lines.append(f"- all of step61..step70 present: {log.get('steps_61_to_70_all_present')} "
                 f"(missing: {log.get('missing_steps_61_to_70')})")
    lines.append(f"- 'Setting global step to 60': {log.get('start_step_line_present')}")
    ...
    for tag in ("step60_extra_state", "step70_extra_state"):
        v = resume.get(tag)
    ...
    # ---------------- step70 manifest ----------------
    m = ["# E1-0 step70 checkpoint manifest", "",
         f"- path: `{CKPT}`",
         f"- produced by: E1-0 `step60 -> ORIGINAL GAP reward -> step70` run",
    ...
    m.append("")
    m.append("The checkpoint is NOT copied into E1-0 (task section 19); only this manifest is kept.")
    with open(os.path.join(MANIFEST, "step70.md"), "w") as f:
        f.write("\n".join(m) + "\n")
