#!/usr/bin/env python3
"""Paired per-prompt comparison of two eval dumps (E1-A vs matched baseline).
...
def main():
    ...
    ap.add_argument("--e1a-dump", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--label", default="e1a_vs_baseline")
    args = ap.parse_args()
    os.makedirs(args.out, exist_ok=True)

    b = load(args.baseline_dump)
    a = load(args.e1a_dump)
    ...
    with open(os.path.join(args.out, "paired_comparison.json"), "w") as f:
        json.dump(out, f, indent=2, default=str)

    o = out["overall"]
    lines = ["# Paired per-prompt comparison (E1-A vs matched baseline)", "",
             f"- paired prompts: **{o['n_paired_prompts']:,}**", "",
             "| metric | E1-A | baseline | delta |", "|---|---|---|---|",
             f"| search rounds | {o['rounds_e1a_mean']:.4f} | {o['rounds_baseline_mean']:.4f} | "
             f"{o['rounds_delta']:+.4f} ({(o['rounds_delta']/max(o['rounds_baseline_mean'],1e-9))*100:+.2f}%) |",
             f"| search queries | {o['queries_e1a_mean']:.4f} | {o['queries_baseline_mean']:.4f} | "
             f"{o['queries_delta']:+.4f} |",
             f"| response chars | {o['chars_e1a_mean']:.1f} | {o['chars_baseline_mean']:.1f} | "
             f"{o['chars_e1a_mean']-o['chars_baseline_mean']:+.1f} |",
             f"| EM | {o['em_e1a_mean']:.4f} | {o['em_baseline_mean']:.4f} | {o['em_delta']:+.4f} |",
             "",
    ...
    for ds, v in out["per_benchmark"].items():
        lines.append(f"| {ds} | {v['n_paired_prompts']:,} | {v['rounds_e1a_mean']:.3f} | "
                     f"{v['rounds_baseline_mean']:.3f} | {v['rounds_delta']:+.3f} | "
                     f"{v['rounds_decreased_rate']*100:.1f}% | {v['rounds_increased_rate']*100:.1f}% | "
                     f"{v['em_delta']:+.4f} |")
    with open(os.path.join(args.out, "paired_comparison.md"), "w") as f:
        f.write("\n".join(lines) + "\n")
    print("\n".join(lines))
    return 0
