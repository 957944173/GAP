"""E1-A reward scorer: success-conditioned group-relative efficiency reward.
...
Design constraints (E1-A_task.md):
...
def _load_original_scorer():
    spec = importlib.util.spec_from_file_location("e1a_original_mhqa_train", _ORIGINAL_MHQA_TRAIN)
...
def get_lambda():
    try:
        return float(os.environ.get("E1A_LAMBDA", DEFAULT_LAMBDA))
...
def compute_score_em_efficiency_batch(
    ...
    **kwargs,
    ...
    for i in range(n):
        ...
        # E1-A invariant: the original scorer's score is the EM (no shaping there)
