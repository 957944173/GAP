...
being inferred from absence of output:
  ...
  * ActorRolloutRefWorker.load_checkpoint : worker side of the resume RPC.
      ...
      - if it prints "entered" but not "returned", the stall is inside the
        worker (already instrumented further down by e1a_checkpoint_patch);
...
def _wrap_method(cls, name):
    ...
    if orig is None or getattr(orig, "_e1a_wrapped", False):
        return False
    label = f"{cls.__name__}.{name}"

    @functools.wraps(orig)
    def wrapper(self, *args, **kwargs):
        print(f"[E1-A diag][worker] {label} entered", flush=True)
        result = orig(self, *args, **kwargs)
        print(f"[E1-A diag][worker] {label} returned", flush=True)
        return result

    wrapper._e1a_wrapped = True
...
def _install():
    ...
    try:
        ...
        print(f"[E1-A diag][worker] fsdp_workers instrumentation failed (non-fatal): {e}", flush=True)
    ...
    try:
        ...
        print(f"[E1-A diag][worker] sharding-manager instrumentation failed (non-fatal): {e}", flush=True)
